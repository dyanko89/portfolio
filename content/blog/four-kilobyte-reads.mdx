...
faq:
  - q: "Why was the Rust disk scanner slow?"
    a: "Three compounding causes: the parallel walk serialized on a shared arena, every file cost three kernel round-trips, and the NTFS fast path read 4 KB per syscall. The per-file syscalls fix alone cut a 2-million-node tree scan to 24.9 seconds cold, 7.1 warm. The MFT reader fix, a buffered reader with parallel decode, took the direct MFT scan from 144.8 seconds to 11.8."
---
