...
- **Server-side association.** Threads and the crate are new per-project JSON files the backend gained (still boundary-pure). When you generate inside a thread, the server parses the job's output, derives each concept's tag from its run-stamp, and files it under that thread — race-free, and it survives a mid-job reload.
